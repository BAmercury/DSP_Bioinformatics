...
for i=1:positions % For each position of the sequence
    ...
    H = [H, H_pos];
    R = [R, 2-(H_pos+e)]; % Calculate Rseq for this position
...
figure(1)
plot(1:positions, R)
title({'Rseq vs. position plot','Generated using custom MATLAB code, e(n)=-0.5'})
xlabel('Position')
ylabel('Bits')
%%
...
%%
% From the two tables, we can see that the peaks are similar, but the bits
% along the y axis are on different levels of magnitude. Adjusting e(n) can
% help us scale the plot to the appropiate magnitude

%%
% Set e(n) = -0.5 and reran the code, here was the generated plot
imshow('updatee.jpg')
%%
% We can see here that the magnitude was scaled up, and it fits more
% closely to the web logo plot
